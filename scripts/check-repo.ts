...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-Visual-Tracking') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-visual-tracking.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-visual-tracking/issues') {
...
function checkReadmes() {
  ...
  if (!readmeJa.startsWith('# TurboWarp-Visual-Tracking\n')) {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/visual-tracking.js')) {
